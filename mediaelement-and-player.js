...
mejs.MediaFeatures = {
	init: function() {
		...
		t.supportsPointerEvents = (function() {
			...
			supports = false;
			try {
				// Trivantis added try/catch saw getComputedStyle return null in Firefox.
			...
			if ((mf.isiPad && t.options.iPadUseNativeControls) || (mf.isiPhone && t.options.iPhoneUseNativeControls)) {
				...
				t.container =
					$('<div id="' + t.id + '" class="mejs-container ' + (mejs.MediaFeatures.svgAsImg ? 'svg' : 'no-svg') +
					  '" role="application" aria-label="' + videoPlayerTitle + '">'+
			...
			if (track !== null && track.isLoaded) {
				for (i=0; i<track.entries.times.length; i++) {
					if (t.media.currentTime >= track.entries.times[i].start && t.media.currentTime <= track.entries.times[i].stop) {
						...
						//TRIVANTIS TAG - LO-3050 make sure to update this block if we update mediaelement library
						if($(t.captionsText).css('background-color')=="rgba(0, 0, 0, 0)")
							updateBG = t;
						...
						if(updateBG)
							$(t.captionsText).css('background-color', "rgba(0, 0, 0, 0)");
